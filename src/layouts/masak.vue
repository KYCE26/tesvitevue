...
// ✅ Tambah/Edit Menu dengan Debugging
const addOrEditMenu = (menu = null) => {
  ...
  Swal.fire({
    ...
    html: `
      ...
      <div id="bahan-container" style="display: flex; flex-direction: column; gap: 10px; border: 1px solid #ddd; padding: 10px; border-radius: 5px; background: #f9f9f9;">
        ${rawMaterials.value.map(material => `
          <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px;">
            <label style="display: flex; align-items: center; gap: 5px; flex: 1;">
              ...
              ${material.nama_bahan} <small>(${material.supplier.nama_supplier})</small>
            </label>
            <input type="number" class="material-amount swal-input" placeholder="Kebutuhan (kg)" style="width: 80px;" 
        ...
        if ((checkbox as HTMLInputElement).checked) {
          ...
          if (!isNaN(kebutuhan) && kebutuhan > 0) {
            selectedMaterials.push({ 
              id_bahan: material.id_bahan, // 🔥 Kirim ID bahan, bukan nama bahan
              kebutuhan 
            });
          }
        }
